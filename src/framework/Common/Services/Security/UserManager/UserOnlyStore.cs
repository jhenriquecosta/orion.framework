...
using Zeus.Helpers;
using IdentityUser = Orion.Framework.App.Services.Domain.Entities.IdentityUser;
...
namespace Orion.Framework.App.Services.Services
{
       ...
       IProtectedUserStore<TUser> where TUser : IdentityUser
    {
        ...
        public UserOnlyStore(IdentityErrorDescriber describer) : base(describer ?? new IdentityErrorDescriber())
        {
        ...
        protected override async Task<TUser> FindUserAsync(string userId,CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            var user = await Users.FirstOrDefaultAsync(u => u.Id.Equals(userId),cancellationToken);
            return user;
        }

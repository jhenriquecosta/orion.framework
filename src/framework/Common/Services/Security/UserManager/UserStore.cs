...
namespace Orion.Framework.App.Services.Services
{
        ...
        IProtectedUserStore<TUser> where TUser : ApplicationUser where TRole : ApplicationRole
    {
        ...
        {
           // _unitOfWork = NHibernateHelper.Instance.GetUnitOfWork(typeof(TUser));
            var _session = Ioc.Create<ISession>();
        ...
        public async Task DettachAsync(TUser user)
        {
            await session.EvictAsync(user);

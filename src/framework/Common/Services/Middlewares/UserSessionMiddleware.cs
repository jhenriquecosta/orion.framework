...
namespace Orion.Framework.App.Services.Services.Middlewares
{
    public class UserSessionMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext httpContext, ILogger<UserSessionMiddleware> logger, IUserSession userSession)
        {
            ...
            try
            {
                ...
                {
                    userSession.UserId = httpContext.User.Claims.Where(c => c.Type == JwtClaimTypes.Subject).First().Value;
                    userSession.UserName = httpContext.User.Identity.Name;
                    userSession.OrganizationCode = 0; // ClaimsHelper.GetClaim<int>(context.User, "tenantid");

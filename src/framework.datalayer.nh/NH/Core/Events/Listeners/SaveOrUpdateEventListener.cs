...
namespace Zeus.NHibernate.Events.Listeners
{
    ...
    public class SaveOrUpdateEventListener : DefaultSaveOrUpdateEventListener
    {
        ...
        protected override async Task<object> PerformSaveOrUpdateAsync(SaveOrUpdateEvent @evt, CancellationToken cancellationToken)
        {
            if (evt.Entity is IEntity)
            {
                Events.CoreEvents.Instance.OnEntitySaving((IEntity)evt.Entity, evt.Session);
            ...
            else
            {
                ...
                eventListenerHelper.OnCreate(evt.Entity);
            }
            return await base.PerformSaveOrUpdateAsync(evt,cancellationToken);
        }
